...
export const MapSearchBar: React.FC<MapSearchBarProps> = ({
  ...
  // Sync initial address when map updates (reverse geocoding from parent)
  useEffect(() => {
  ...
  const handleSearchTextChange = (text: string) => {
    ...
    if (text.length > 2) {
      ...
      searchTimeoutRef.current = setTimeout(async () => {
        try {
          const features = await geocodingService.searchAddress(text);
  ...
  const mapFeatureToSearchResult = (feature: GeocodingFeature): SearchResult => {
    ...
    const title = feature.properties?.name || feature.place_name?.split(',')[0] || 'Unknown';
  ...
  const handleSelectItem = (item: SearchResult) => {
    Keyboard.dismiss();
    setQuery(item.subtitle); // Update text input to full address
...
const styles = StyleSheet.create({
  container: {
    ...
    left: 16,
    right: 16,
    zIndex: 100, // Ensure it floats above map
  },
  inputWrapper: {
    flexDirection: 'row',
    alignItems: 'center',
